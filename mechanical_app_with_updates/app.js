$(document).ready(function() {
  ...
  function applyPhysics(object, deltaTime) {
    const gravity = -9.81; // Basic gravity
    object.position.y += object.velocityY * deltaTime;
    object.velocityY += gravity * deltaTime;
    if (object.position.y <= 0) {
      object.position.y = 0;
      object.velocityY *= -0.5; // Simple bounce
    }
  }
  cube.velocityY = 0; // Initialize velocity
  ...
  let temperature = Array(100).fill(20); // Initial temperatures
  ...
  function reconstructPath(cameFrom, current) { let path = [current]; while (current in cameFrom) { current = cameFrom[current]; path.push(current); } return path.reverse(); }
  function getNeighbors(pos) { return [pos - 1, pos + 1]; } // Simplified neighbors

  // 6. Predictive Maintenance (Basic Failure Prediction)
  function failurePrediction(usage) {
    const failureProbability = 1 - Math.exp(-usage / 1000); // Basic exponential model
